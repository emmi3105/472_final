...
# Execute the queries
endurance_score_result_rs100 <- dbGetQuery(db, endurement_score_query_rs100)
endurance_score_result_spotify <- dbGetQuery(db, endurement_score_query_spotify)
endurance_score_result_fm <- dbGetQuery(db, endurement_score_query_fm)
...
endurance_score_result_fm$Latest_Album_Release <- NA
endurance_score_result_fm$On_Tour <- NA
...
```{r latest album release vs. endurance, fig.width=10, fig.height=4}
...
latest_album_release_plot <- ggplot(endurance_score_data, 
...
```{r plot riaa certifications vs endurance}
...
# Plot C: On Tour vs. Endurance
tour_plot <- ggplot(endurance_score_data, 
                                aes(x = Spotify_Endurance, y = LastFM_Endurance, color = Appearance, shape=On_Tour)) +
  geom_point(size = 1) +
  # Add titles
  labs(title = "Latest Album Release Date vs. Spotify Endurance",
       x = "Spotify Endurance",
...
# Plot C: On Tour vs. Endurance
tour_plot <- ggplot(endurance_score_data, aes(fill=Appearance, y=LastFM_Endurance, x=On_Tour)) + 
