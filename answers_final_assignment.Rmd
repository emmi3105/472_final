...
We calculate an endurance score.

We scaled it so that the artist with the highest endurance will have an endurance score of 100% and an endurance of 0 is translated to a score of 0%.

We will compare this score 

## 2. Data

Firstly, we created a relational database to which we later add the data. The primary key of the tables in the database is the variable `Spotify_Artist_ID`, which refers to the unique ID each artist is assigned in the Spotify API. Overall, we used four different data sources that we will briefly elaborate on in the following section.
...
From the [Rolling Stone's 100 Greatest Artists, 2010](https://www.rollingstone.com/music/music-lists/100-greatest-artists-147446/), we extracted the names of the 100 greatest artists [from now on referred to as RS-T100] ranked by the Rolling Stones Magazine in 2010.

### Spotify API

We used the [Spotify Web API](https://developer.spotify.com/documentation/web-api) to get further information on the RS-T100, such as the Spotify artist ID, the Followers and Popularity on Spotify, and the date of the latest album release.

Furthermore, we extracted the same variables for the artists that appeared in the playlist [Top Artists of 2023 USA](https://open.spotify.com/playlist/37i9dQZF1DX4dwwIoKH0j7) [from now on referred to as Spotify Top 50]. We chose the US playlist, as, firstly, the US is a major market on Spotify, secondly, the Rolling Stones is a US-American magazine, and thirdly, most artists that appeared in the RS-T100 are US-American. Thereby, focusing on the US market on Spotify might give the most realistic comparison to the RS-T100.

Note that for all but one observations, the data was properly extracted. For "Parliament Funkadelic", the get request extracted an incorrect Spotify ID. Thereby, we manually changed this single ID to the correct one.

### LastFM API

The [LastFM API](https://www.last.fm/de/api) holds additional metrics on music artists, from which we created the variables Playcount, Listeners and a binary variable that indicates whether the artist is currently on tour, for both, the RS-T100 and the Spotify Top 50.

Furthermore, we also extracted the top 50 artists as ranked by the charts on LastFM [from now on referred to as LastFM Top 50]. All of the previously mentioned variables were created for the LastFM Top 50 as well.

### RIAA Data (Wikipedia)

Finally, we added another data source to our existing data. We wanted to not only compare the RS-T100 to the most streamed or listened to artists on platforms such as Spotify and LastFM, but we also wanted to know whether the endurance might be related to awards and certifications. Therefore, we used the Recording Industry Association of America (RIAA) award certification. We extracted information on the RIAA certification of music artists in the United States, which is based on the sales of albums through retail and other ancillary markets. As [RIAA does not allow webscraping](https://www.riaa.com/privacy-policy-and-terms-of-use/), we extracted the names of the hundred most certified music artists [from now on referred to as RIAA Top 100] from [Wikipedia](https://en.wikipedia.org/wiki/List_of_highest-certified_music_artists_in_the_United_States).

In the end, we had created four tables that each held the same information on the artists, as shown below.
...
# 3) Get the Spotify playlist "Top Artists of 2023, USA"
...
Potentially get rid of certified_singles_df?
...
The following data frames were written to the relational database.

1. RS Top 100 Artists
...
3. Spotify: Top 50 Groups of 2023 (potentially delete)
...
4. LastFM: Top 50 Artists Charts
...
5. RIAA: Highest-certified Music Artists in the US based on certifications of albums
...
6. RIAA: Highest-certified Music Artists in the US based on certifications of singles

```{r call check_table on "certified_singles_df"}

# Call check_table on "certified_singles_df"
...
variable_table <- data.frame(
  ...
  Explanation = c("Name of the artist",
                  ...
                  "RIAA album units: 1,500 streams = 1 album unit and 10 track sales = 1 album unit",
                  "Binary indicator: 1 if the artist appears in the RS-T100, 0 otherwise"),
  Source = c("Rolling Stones/Spotify/Last_FM/RIAA",
             "Spotify",
             "Rolling Stones",
             "Spotify",
             "Spotify",
...
We calculate two *Endurance Scores* for the RS-T100 as well as the Spotify Top 50, the LastFM Top 50 and the RIAA Top 100: 

$Spotify\_Endurance = Spotify\_Popularity * Spotify\_Followers$

$LastFM\_Endurance = LastFM\_Playcount * LastFM\_Listeners$

Then, we scale the endurance scores so that the artist with the highest endurance, which is Taylor Swift on Spotify and LastFM, is scaled to 100%. An endurance of 0 is scaled to 0%.^[As of 23. December 2023, Taylor Swift had 109'341'734 monthly listeners on Spotify.] 

#### On a scale of 1 to Taylor Swift ...?

Using the endurance scores as calculated above, we plot the Spotify_Endurance against the LastFM_Endurance of the RS-T100, the LastFM Top 50, and the Spotify Top 50.

Unsurprisingly, the two endurance scores seem to be positively correlated. Furthermore, Taylor Swift has the highest endurance scores by large, none of the RS-T100 can compete with her popularity.
...
In the violin plots below, the distributions of the endurance scores for the three groups are visualised. Clearly, the RS-T100 have, on average, a significanlty lower endurance on Spotify and LastFM compared to the most popular artists of 2023. However, the distribution is quite skewed toward larger values, showing outliers with values up to 75% (58%) on Spotify (LastFM). This raises two questions: 

Which of the RS-T100 have a significantly higher endurance than the average, and what explains that?
...
Potentially, some of the RS-T100 also appear in the Spotify or LastFM Top 50. The venn diagram below shows these intersections.
...
Visibly, there are a few RS-T100 that also appear to be within the most popular artists of 2023. In the following section, we approach the question what could possibly give these artists stronger endurance.
