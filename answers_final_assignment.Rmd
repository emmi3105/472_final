...
endurement_score_query_fm <-  "
  SELECT Spotify_Artist_ID, Followers, Popularity, Listeners AS Listeners_LastFM, Playcount AS Playcount_LastFM
...
endurement_score_result_fm <- dbGetQuery(db, endurement_score_query_fm)
...
endurement_score_result_fm$Appearance <- "Last FM Artist Charts"
endurement_score_result <- rbind(endurement_score_result_rs100, endurement_score_result_spotify, endurement_score_result_fm)
...
endurement_score_data <- endurement_score_data %>%
...
endurement_plot <- ggplot(endurement_score_data,
                          aes(x = Spotify_Endurement, y = LastFM_Endurement, color = Appearance)) +
  geom_point(size = 1) +
...
```{r plot the endurement distribution, fig.width=10}

endurement_distribution_fm <- ggplot(endurement_score_data, aes(x=Appearance, y=LastFM_Endurement, fill=Appearance)) +
  geom_violin() +
  stat_summary(fun=mean, geom="point", shape=23, size=3, fill="white") +
  stat_summary(fun=mean, geom="text", aes(label=sprintf("%.2f", after_stat(y))),
               vjust=-0.5, hjust=-0.5, size=3, color="black") +
  labs(title = "LastFM Endurement Distribution",
       x = "Appearance",
       y = "LastFM Endurement") +
  theme_minimal() +
  # Adjust the font size for the title and the axes
...
endurement_distribution_spotify <- ggplot(endurement_score_data, aes(x=Appearance, y=Spotify_Endurement, fill=Appearance)) +
  geom_violin() +
  stat_summary(fun=mean, geom="point", shape=23, size=3, fill="white") +
  stat_summary(fun=mean, geom="text", aes(label=sprintf("%.2f", after_stat(y))),
               vjust=-0.5, hjust=-0.5, size=3, color="black") +
  labs(title = "Spotify Endurement Distribution",
       x = "Appearance",
       y = "Spotify Endurement") +
  theme_minimal() +
  # Adjust the font size for the title and the axes
  theme(
    axis.text = element_text(size = 8),     
...
combined_endurement_distribution <- plot_grid(
  endurement_distribution_spotify + theme(legend.position = "none"),
  endurement_distribution_fm + theme(legend.position = "none"),
  ncol = 2, nrow = 1, align = "v", rel_widths = c(1, 1)
)

# Display the combined plot
combined_endurement_distribution
```
...
Venn diagramm with the three samples
